...
FOLDER_NAME=${PWD##*/}
CRATE_NAME="event_timer" #$FOLDER_NAME assume crate name is the same as the folder name
...
if [[ "${FAST}" == false ]]; then
  ...
  wasm-opt "docs/${CRATE_NAME}_bg.wasm" -O2 -o "docs/${CRATE_NAME}_bg.wasm" --all-features -g # add -g to get debug symbols
fi
